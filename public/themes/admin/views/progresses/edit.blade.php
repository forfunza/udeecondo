{{ Form::open(array('action' => array('ProgressesController@update',$progress->id), 'method' => 'put', 'class' => 'form-horizontal bucket-form' , 'enctype' => 'multipart/form-data')) }}
	<section class="panel">
		...
		<div class="panel-body">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-md-10">
						<div class="fileupload fileupload-{{ !empty($progress->image) ? 'exists' : 'new'}}" data-provides="fileupload">
							<div class="fileupload-new thumbnail" style="width: 200px; height: 150px;">
								<img src="http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=image" alt="">
							</div>
							<div class="fileupload-preview fileupload-exists thumbnail" style="max-width: 200px; max-height: 150px; line-height: 20px;">
								@if(!empty($progress))
								<img src='{{ !empty($progress->image) ? $progress->image : "" }}'>
								@endif
							</div>
							<div>
				...
				<div class="tab-content">
					...
					<div id="data-{{ $language->id }}" class="tab-pane {{ $language->id == 1 ? 'active' : ''}}">
						<div class="form-group">
							<label class="col-sm-2 control-label">หัวข้อ ({{ $language->name }})</label>
							<div class="col-sm-6">
								<input name="progress_description[{{ $language->id }}][name]" value="{{ $language->progresses->find($progress->id)->pivot->name }}" type="text" class="form-control" required>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-md-10">
								<textarea name="progress_description[{{ $language->id }}][description]"  class="ckeditor form-control" rows="6">{{ $language->progresses->find($progress->id)->pivot->description }}</textarea>
							</div>
						</div>
					</div>
					@endforeach
