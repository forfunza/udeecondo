{{ Form::open(array('action' => array('NewsController@update', $news->id), 'method' => 'put', 'class' => 'form-horizontal bucket-form' , 'enctype' => 'multipart/form-data')) }}
	<section class="panel">
		...
		<div class="panel-body">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<div class="col-md-10">
						<div class="fileupload fileupload-{{ !empty($news->image) ? 'exists' : 'new'}}" data-provides="fileupload">
							<div class="fileupload-new thumbnail" style="width: 200px; height: 150px;">
								<img src="http://www.placehold.it/200x150/EFEFEF/AAAAAA&amp;text=image" alt="">
							</div>
							<div class="fileupload-preview fileupload-exists thumbnail" style="max-width: 200px; max-height: 150px; line-height: 20px;">
								@if(!empty($news))
								<img src='{{ !empty($news->image) ? $news->image : "" }}'>
								@endif
							</div>
							<div>
				...
				<div class="tab-content">
					...
					<div id="data-{{ $language->id }}" class="tab-pane {{ $language->id == 1 ? 'active' : ''}}">
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input name="news_description[{{ $language->id }}][name]" value="{{ $language->news->find($news->id)->pivot->name }}" type="text" class="form-control" required>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-md-10">
								<textarea name="news_description[{{ $language->id }}][description]"  class="wysihtml5 form-control" rows="9">{{ $language->news->find($news->id)->pivot->description }}</textarea>
							</div>
						</div>
					</div>
					@endforeach
